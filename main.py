...
class MyApp(App):
    ...
    # Setzt die Größe des Spielfeldes und die minimale Größe die das Fenster besitzen muss
    def build(self):
    ...
    # Animiert das Bild als Reaktion auf entsprechende Aktion
    def on_start(self):
    ...
    # Irgendeine Magic, um den Bildschirm regelmäßig zu aktualisieren
    ...
    # Option zum Auswählen des Modus anzeigen
    ...
    # Zeichnet das Spielfeld und macht in jedem einzelnen Quadrat einen Button um das Feld anklickbar zu machen
    ...
    # Auswahl vom X oder O abhängig vom aktuellen Spielstand
    def choose_img(self, samples):
        x_img = ''
        o_img = ''
        if ('0-0' in samples and '0-1' in samples and '0-2' in samples) or ('1-0' in samples and '1-1' in samples and '1-2' in samples) or ('2-0' in samples and '2-1' in samples and '2-2' in samples):
            x_img = 'xch'
            o_img = 'och'
        elif ('0-0' in samples and '1-0' in samples and '2-0' in samples) or ('0-1' in samples and '1-1' in samples and '2-1' in samples) or ('0-2' in samples and '1-2' in samples and '2-2' in samples):
            x_img = 'xcv'
            o_img = 'ocv'
        elif '0-0' in samples and '1-1' in samples and '2-2' in samples:
            x_img = 'xc'
            o_img = 'oc'
        elif '0-2' in samples and '1-1' in samples and '2-0' in samples:
            x_img = 'xc2'
            o_img = 'oc2'
        return [x_img, o_img]
